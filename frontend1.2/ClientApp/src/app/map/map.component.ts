...
export class MapComponent {
  ...
  //click sobre els punts de les estacions
  onClick(data) {
    ...
    $.ajax({ async: false, data: { "idEstacio": data.sourceTarget.feature.idEstacio }, type: "GET", url: "http://localhost:29164/estacions/FindId", dataType: "json" })

    let panelHtml = '<html>' + '</br>' + `<h4> CURRENT CONDITIONS </h4>` + '</br>' + `<p> Outside Temperature ${geoJson.outTemp}ºC </p>` +
      ...
      '<button type = "button" id="btn" onclick = $.ajax({ async: false, data: { "idEstacio": data.sourceTarget.feature.idEstacio }, type: "GET", url: "http://localhost:29164/estacions/FindId", dataType: "json"})> Comprovar històric </button>' + '</html>'
